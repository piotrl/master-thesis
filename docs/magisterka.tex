...
\epigraph{Without deviation from the norm, progress is not possible}{\textit{Frank Zappa}}
    ...
    Podczas pisania tego wstępu słucham playlisty odpowiednio dobranej pod tę czynność
    ...
    Gdy programuję, zwykle słucham muzyki, jednak innej niż podczas pisania ---
    ...
    więszkość badanych słucha muzyki, duży odsetek preferuje skrajną ciszę, natomiast niewielu preferuje rozpraszacze w tle:
    \begin{itemize}
    ...
    W tej pracy próbuję zweryfikować te założenia przez stworzenie aplikacji, którą analizuje moje własne aktywności i muzykę, którą słucham.
    Jednym z mierników, którym próbuję się posłużyć jest Attention Span
    ...
    % TODO: Wspomnieć o starcie pracy nad aplikacji w NodeJS / MongoDB

    Opisuję pełny proces budowania aplikacji i analizy danych.
    Począwszy od mechanizmu importu danych z zewnętrznych źródeł, w którym charakteryzuję dostawców API
...
\chapter{Import danych}
    ...
    oba są popularne w swojej klasie oraz dostarczają publiczne API dla danych użytkownika.
        ...
        \section*{Last.fm}
            ...
            Serwis by korzystać z API wymaga rejestracji aplikacji do której jest przypisany klucz.
            Po tym jest dostępne szereg metod REST API \cite{lastfm:apidoc}, dla mnie szczególnie istotne są te
            związane z odtworzeniami użytkownika oraz szczegółowych informacji o piosenkach i artystach.

            Szczegółowe informacje na temat artysty lub piosenki dostępne są po podaniu ich MBID\cite{lastfm:mbid} lub nazwy artysty.
...
\section{Opis zebranych danych}
    ...
    \begin{figure}
        ...
        \label{fig:Schemat relacji tabel z Last.fm}
    \end{figure}
