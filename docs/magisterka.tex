...
\begin{abstract}
    ...
    wdrożoną na chmurze Digital Ocean pod adresem \url{mgr.piotrl.net}
...
\epigraph{Without deviation from the norm, progress is not possible}{\textit{Frank Zappa}}
    ...
    \url{open.spotify.com/user/exmortisinvictus/playlist/69GtfS8diB8bUHQ74V4yuP}}
    ...
    Wideo obejrzano ponad 400 tysięcy razy, a koncepcja została rozwinięta w projekcie Focus@Will
    \footnote{Focus@Will --- serwis sugerujący szereg playlist pomagający zwiększyć produktywność. \url{en.wikipedia.org/wiki/Focus@Will}}.
    ...
    często nazywane flow. \url{en.wikipedia.org/wiki/Attention_span}}.
...
\chapter{Architektura}
    ...
    Każdą zmianię w strukturze bazy danych należy dodać jako nowy plik z migracją, w katalogu projektu:
...
% TODO: Bibliografia TDD
Część kodu była napisana metodologią TDD
\footnote{TDD --- Test Driven Development},
głównie metody łączące i prasujące dane z zewnętrznych API oraz zawierająe logikę samego mechanizmu agregacji.

Metody kontrollerów nie były testowane integracyjnie,
a operacje na bazie danych były testowane w podstawowym stopniu umożliwającym sprawdzenie niepustości wyników,
dlatego kod API ma stosunkowo niskie pokrycie operacji 12\%.
...
\section{Stos technologiczny}
    ...
    Deployment aplikacji przygotowano na VPS\footnote{VPS --- Virtual Private Server} w chmurze
    DigitalOcean\footnote{DigitalOcean, Inc. --- Trzeci największy na świecie dostawca hostingów VPS \cite{digitalocean:growth}. \url{http://digitalocean.com/}}
    na maszynie zlokalizowanej we Frankfurcie, z 1GB RAM, przydzielonym jednym procesorem i 20GB dysku SSD.
    Środowisko skonfigurowano pod kontrolą systemu Ubuntu 16.04\cite{digitalocean:server-setup},
    aplikacja działa w konterzenrze Tomcat 8.5\cite{digitalocean:tomcat-setup}, a dane przechowywane są w PostgreSQL 9.5\cite{digitalocean:postgres-setup}.
